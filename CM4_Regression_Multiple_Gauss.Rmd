...
### Assumptions:
  * (H1) $rg(\mathbf{X}) = p$
  * (H2) $\boldsymbol{\epsilon} \sim  N (\mathbf{0}, \sigma^2 \mathbf{I}_n)$ 
...
y_i 
\underset{\text{iid}}{\operatorname{\sim}}
 N (\mathbf{x}^i \boldsymbol{\beta}, \sigma^2) 
\quad 
\text{for}
\quad
1 \leq i \leq n
...
**Property**:  
If $\mathbf{X} \sim  N (\boldsymbol{\mu}, \mathbf{\Sigma})$,
then for any $\mathbf{a}$ matrix and $\mathbf{b}$ vector,  
$$
\mathbf{Y} = \mathbf{a}\mathbf{X} + \mathbf{b} 
\sim  N (\mathbf{a}\boldsymbol{\mu} + \mathbf{b}, \mathbf{a}\mathbf{\Sigma}\mathbf{a}^T).
$$


**Property**:  
Let $\mathbf{X} \sim  N (\boldsymbol{\mu}, \mathbf{\Sigma})$, with $\mathbf{\Sigma}$ invertible.
...
**Definition**:  
Let $X_1, \dotsc, X_p$ be $p$ standard normal iid rv : $X_i \sim  N (0, 1)$.
$$
...
$\mathbf{X}$ is a Gaussian vector $\mathbf{X} \sim  N (\boldsymbol{\mu}, \mathbf{\Sigma})$.
with $\mathbf{\Sigma}$ is invertible, so:
$$
\mathbf{Y} = \mathbf{\Sigma}^{-1/2}(\mathbf{X} - \boldsymbol{\mu})
\sim  N (\mathbf{0}, \mathbf{I}_p).
$$
...
* $\mathbf{Y}$ a Gaussian vector $\mathbf{Y} \sim  N (\boldsymbol{\mu}, \sigma^2\mathbf{I}_n)$;
* $M$ a subspace of $\mathbb{R}^n$ of dimension $p$;
* $\mathbf{P}$ the orthogonal projection matrix on $M$;
* $\mathbf{P}^{\bot} = \mathbf{I}_n - \mathbf{P}$ the orthogonal projection matrix on $   M ^{\bot}$.

Then we have:

(i)
$\mathbf{P}\mathbf{Y} \sim  N (\mathbf{P}\boldsymbol{\mu}, \sigma^2\mathbf{P})$
and
$\mathbf{P}^{\bot}\mathbf{Y} \sim  N (\mathbf{P}^{\bot}\boldsymbol{\mu}, \sigma^2\mathbf{P}^{\bot})$;
...
* $\mathbf{Y}$ a Gaussian vector $\mathbf{Y} \sim  N (\boldsymbol{\mu}, \sigma^2\mathbf{I}_n)$; 
* $\mathbf{P}$ the orthogonal projection matrix on $M$ dim $p$;
* $\mathbf{P}^{\bot} = \mathbf{I}_n - \mathbf{P}$ the orthogonal projection matrix on $   M ^{\bot}$.
(i)
$\mathbf{P}\mathbf{Y} \sim  N (\mathbf{P}\boldsymbol{\mu}, \sigma^2\mathbf{P})$
and
...
$$
\mathbf{P}\mathbf{Y}
\sim  N (\mathbf{P}\boldsymbol{\mu}, \mathbf{P}[\sigma^2\mathbf{I}_n]\mathbf{P}^T)
$$
...
\mathbf{Z}
= \mathbf{U}^T \mathbf{Y}
\sim  N (\mathbf{U}^T\boldsymbol{\mu}, \mathbf{U}^T[\sigma^2\mathbf{I}_n]\mathbf{U} = \sigma^2\mathbf{I}_n).
$$
...
$$
\mathbf{Z}
\sim  N (\mathbf{U}^T\boldsymbol{\mu}, \sigma^2\mathbf{I}_n)
\quad
\text{hence}
\quad
\mathbf{Z}_p
\sim  N ((\mathbf{U}^T\boldsymbol{\mu})_p, \sigma^2\mathbf{I}_p)
$$
...
* $\mathbf{Y}$ a Gaussian vector $\mathbf{Y} \sim  N (\mathbf{X}\boldsymbol{\beta}, \sigma^2\mathbf{I}_n)$;
* $M(\mathbf{X}) = \text{span}\{\mathbf{x}_1, \dotsc, \mathbf{x}_p\}$;
* $\mathbf{P}^{\mathbf{X}} = \mathbf{X}(\mathbf{X}^{T}\mathbf{X})^{-1}\mathbf{X}^{T}$ orthogonal projection on $M (\mathbf{X})$;
...
\hat{\beta}_k
\sim
 N (\beta_k, \sigma^2 [(\mathbf{X}^T\mathbf{X})^{-1}]_{kk})
$$
...
\mathbb{P}[\text{Reject} |  H_0 \text{ true}] \leq \alpha
\qquad\qquad\qquad\qquad\qquad\\
\qquad\qquad\qquad \iff  {R}_\alpha = \left\{ t \in \mathbb{R}  ~|~ |t| \geq t_{n-p}(1 - \alpha/2) \right\}
$$

$$
\text{p value:}\qquad\qquad\qquad\qquad\quad
p = \mathbb{P}_{ H_0}[|T_k| > T_k^{obs}]
$$
...
$$
with
$\epsilon_{n+1} \sim  N (0, \sigma^2)$, independent from all $(\epsilon_{i})_{1\leq i \leq n}$.
