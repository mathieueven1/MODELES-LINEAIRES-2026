...
>* **Goal**: Test hypothesis (H2): $\boldsymbol{\epsilon} \sim  {N}(\mathbf{0}, \sigma^2 \mathbf{I}_n)$.
...
\sim  {N}(\mathbf{0}, \sigma^2 (\mathbf{I}_n - \mathbf{P}^{\mathbf{X}}))
$$
...
If $n$ is large, then $ {T}_{n - p -1} \approx  {N}(0,1)$.

$\to$ QQ-plot on the Studentized residuals (quantile-to-quantile diagram).
...
If $\epsilon_i \sim  {N}(0, \sigma^2 )$ iid, then
$$
\hat{\beta}_2 \sim  {N}(\beta_2, \sigma^2 [(\mathbf{X}^T\mathbf{X})^{-1}]_{22})
$$
