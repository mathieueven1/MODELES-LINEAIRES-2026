...
header-includes: \usepackage{amsmath,amssymb,amsfonts}
---
...
If $X \sim \mathscr{N}(\mu, \sigma^2)$, then for any $(a, b) \in \mathbb{R}^2$,  
$$
Y = aX + b \sim \mathscr{N}(a\mu + b, a^2\sigma^2).
$$
...
y_i 
\underset{\text{iid}}{\operatorname{\sim}}
\mathscr{N}(\beta_0 + \beta_1 x_i, \sigma^2) 
\quad 
\text{for}
\quad
1 \leq i \leq n
...
$$
\hat{\beta}_0 \sim \mathscr{N}\left(\beta_0, \frac{\sigma^2}{n}\left(1 + \frac{\bar{x}^2}{s_{\mathbf{x}}^2}\right)\right)
\quad
\hat{\beta}_1 \sim \mathscr{N}\left(\beta_1, \frac{\sigma^2}{n}\frac{1}{s_{\mathbf{x}}^2}\right)
$$
...
Let $X_1, \dotsc, X_p$ be $p$ standard normal iid rv : $X_i \sim \mathscr{N}(0, 1)$.
Then 
$$
...
Let $Z$ be a standard normal rv : $Z \sim \mathscr{N}(0, 1)$,  
and $X$ a chi squared rv with $p$ degrees of freedom: $X \sim \chi^2_p$,
$Z$ and $X$ independent.
Then 
$$
T = \frac{Z}{\sqrt{X/p}} \sim \mathscr{T}_p
$$ 
...
y_{n+1} - \hat{y}_{n+1}
\sim
\mathscr{N}\left(0, \sigma^2 \left(1 + \frac{1}{n} + \frac{1}{ns_{\mathbf{x}}^2} (x_{n+1} - \bar{x})^2\right)\right)
$$
...
<!-- If $\mathbf{X} \sim \mathscr{N}(\boldsymbol{\mu}, \mathbf{\Sigma})$, -->
<!-- then for any $\mathbf{a}$ matrix and $\mathbf{b}$ vector,   -->
<!-- $$ -->
<!-- \mathbf{Y} = \mathbf{a}\mathbf{X} + \mathbf{b}  -->
<!-- \sim \mathscr{N}(\mathbf{a}\boldsymbol{\mu} + \mathbf{b}, \mathbf{a}\mathbf{\Sigma}\mathbf{a}^T). -->
<!-- $$ -->
...
* with $\mathscr{F}^2_{n-2}$ the **Fisher** distribution.
...
Then 
$$
F = \frac{U_1/p_1}{U_2/p_2} \sim \mathscr{F}^{p_1}_{p_2}
$$ 
...
When $p_2$ goes to infinity, $\mathscr{F}^{p_1}_{p_2}$ converges in distribution
