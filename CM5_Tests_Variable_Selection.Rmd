...
\qquad\qquad\qquad \iff R_\alpha = \left\{ t \in \mathbb{R}  ~|~ |t| \geq t_{n-p}(1 - \alpha/2) \right\}
$$
...
Reject Region: $\mathbb{P}[\text{Reject} | H_0 \text{ true}] \leq \alpha$
...
with $F^p_{n-p}$ the **Fisher** distribution.
...
Then 
$$
F = \frac{U_1/p_1}{U_2/p_2} \sim F^{p_1}_{p_2}
$$ 
...
$$
\text{If } \mathbf{X} \sim N(\boldsymbol{\mu}, \mathbf{\Sigma})
\quad
\text{then}
\quad
...
\frac{U_1/p_1}{U_2/p_2} \sim F^{p_1}_{p_2}
$$ 
...
\qquad\qquad\qquad\qquad\qquad\\
\qquad\qquad\qquad
\!\iff\! R_\alpha = \left\{ f \in \mathbb{R}_+  ~|~ f \geq f^p_{n-p}(1 - \alpha) \right\}
$$

p value:$\qquad\qquad p = \mathbb{P}_{H_0}[F > F^{obs}]$
...
>* $H_0: \beta_1 = \cdots = \beta_p = 0$ i.e. the model is useless.

>* If $\|\hat{\boldsymbol{\epsilon}}\|^2$ is **small**, then the error is **small**,  
and $F$ tends to be **large**, i.e. we tend to **reject** $H_0$:  
the model is useful.

>* If $\|\hat{\boldsymbol{\epsilon}}\|^2$ is **large**, then the error is **large**,  
and $F$ tends to be **small**, i.e. we tend to **not reject** $H_0$:  
...
* $\mathbf{y}$ **not too far** from $M(\mathbf{X})$.
...
* $\mathbf{y}$ **almost orth.** to $M(\mathbf{X})$.
...
$$
\begin{aligned}
H_0&: \beta_{p_0 + 1} = \cdots = \beta_p = 0 &\text{vs}\\
H_1&: \exists~k\in \{p_0+1, \dotsc, p\} ~|~ \beta_k \neq 0
\end{aligned}
$$
...
Let $\mathbf{P}$ projection on $M(\mathbf{X})$ and
$\mathbf{P}_0$ projection on $M(\mathbf{X}_0)$
...
&\in M(\mathbf{X}_0)^\bot \cap M(\mathbf{X})
\end{aligned}
$$
...
If $H_0$ is true, then $\mathbf{P}_0^\bot \mathbf{P}\mathbf{X}\boldsymbol{\beta} = 0$, hence:
...
$$
\begin{aligned}
H_0&: \beta_{2} = \cdots = \beta_p = 0 &\text{vs}\\
H_1&: \exists~k\in \{2, \dotsc, p\} ~|~ \beta_k \neq 0
\end{aligned}
$$
...
i.e. as an element of $M(\mathbf{X})$.
...
\quad
\boldsymbol{\epsilon}_{\eta} \sim N(\mathbf{0}, \sigma^2\mathbf{I}_n).
$$
...
i.e. as an element of $M(\mathbf{X}_{\eta})$.
...
* $\boldsymbol{\mu} \in M(\mathbf{X}_{\eta})$ so that 
