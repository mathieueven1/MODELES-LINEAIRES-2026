...
author: "Mathieu Even - Amélie Vernay"
date: "22/04/2024"
output:
...
* $y^f_{i} \sim  {N}(\mu_f, \sigma^2)$ iid
* $y^m_{j} \sim  {N}(\mu_m, \sigma^2)$ iid

**Test**:

$ {H}_0$: $\mu_f = \mu_m$ vs $ {H}_1$: $\mu_f \neq \mu_m$
...
\underset{ {H}_0}{\sim}  {T}_{n_f + n_m - 2}
$$
...
* $y^f_{i} = \mu_f + \epsilon_i^f$, with $\epsilon^f_i \sim  {N}(0, \sigma^2)$ iid
* $y^m_{j} = \mu_m + \epsilon_j^m$, with $\epsilon^m_j \sim  {N}(0, \sigma^2)$ iid
...
\quad \text{with} \quad \boldsymbol{\epsilon} \sim  {N}(\mathbf{0}, \sigma^2 \mathbf{I}_n)
$$
...
the test
$$
 {H}_0: \mu_f = \mu_m \quad \text{vs} \quad  {H}_1: \mu_f \neq \mu_m
$$

becomes:
$$
 {H}_0: \beta_2 = 0 \quad \text{vs} \quad  {H}_1: \beta_2 \neq 0
$$
...
* $n_f$ female adelie penguins, $n_m$ male adelie penguins
* $y^f_{i}$: mass (g) of a female adelie penguin, $1 \leq i \leq n_f$, $y^f_{i} \sim  {N}(\mu_f, \sigma^2)$ iid
* $y^m_{j}$: mass (g) of a male adelie penguin, $1 \leq j \leq n_m$, $y^m_{j} \sim  {N}(\mu_m, \sigma^2)$ iid
* $ {H}_0$: $\mu_f = \mu_m$ vs $ {H}_1$: $\mu_f \neq \mu_m$
...
* $y^s_{i} \sim  {N}(\mu_s, \sigma^2)$ iid for any species $s$.

**Pairwise Tests**:

$ {H}^{s_1,s_2}_0$: $\mu_{s_1} = \mu_{s_2}$ vs $ {H}^{s_1,s_2}_1$: $\mu_{s_1} \neq \mu_{s_2}$  
for any pair $(s_1,s_2)$ of species.
<!-- $\to$ ${|S| \choose 2 }$ tests. -->

**Global Tests** can we do:

$ {H}_0$: $\mu_a = \mu_c = \mu_g$ vs $ {H}_1$: $\exists s_1, s_2 \in S ~|~ \mu_{s_1} \neq \mu_{s_2}$ ?
...
$$
 {H}^{k}_0: \beta_{k} = 0 \quad{ vs } \quad  {H}^{k}_1: \beta_{k} \neq 0
$$

is the same as the test of the **difference** between groups $1$ & $k$:

$$
 {H}^{1,k}_0: \mu_{1} = \mu_{k} \quad{ vs } \quad  {H}^{1,k}_1: \mu_{1} \neq \mu_{k}
$$
...
$$
 {H}_0: \beta_{2} = \beta_3 = \cdots = \beta_{K} = 0 \quad{ vs } \quad  {H}_1: \exists k | \beta_{k} \neq 0
$$

is the same as the global test of mean **equality of all groups**:

$$
 {H}_0: \mu_{1} = \mu_2 = \cdots = \mu_{K} \quad{ vs } \quad  {H}_1: \exists k, k' | \mu_{k} \neq \mu_{k'}
$$
...
$$
\begin{aligned}
 {H}_0&: \beta_{1} = \cdots = \beta_K = 0 &\text{vs}\\
 {H}_1&: \exists~k\in \{1, \dotsc, K\} ~|~ \beta_k \neq 0
\end{aligned}
$$
...
$$
\begin{aligned}
 {H}_0&: \beta_{p_0 + 1} = \cdots = \beta_p = 0 &\text{vs}\\
 {H}_1&: \exists~k\in \{p_0+1, \dotsc, p\} ~|~ \beta_k \neq 0
\end{aligned}
$$
...
$$
\begin{aligned}
 {H}_0&: \mathbf{P}^{\mathbf{X}_{p_0}}\mathbf{y} = \mathbf{P}^{\mathbf{X}}\mathbf{y}  &\text{vs}\\
 {H}_1&: \mathbf{P}^{\mathbf{X}_{p_0}}\mathbf{y} \neq \mathbf{P}^{\mathbf{X}}\mathbf{y}
\end{aligned}
$$
...
$$
\begin{aligned}
 {H}_0&: \text{"Larger model 2 does not bring any information compared to model 1"}\\
 {H}_1&: \text{"Larger model 2 does bring some information compared to model 1"}
\end{aligned}
$$
...
$$
\begin{aligned}
 {H}_0&: \mathbf{P}^{\mathbf{X}_{\eta_1}}\mathbf{y} = \mathbf{P}^{\mathbf{X}_{\eta_2}}\mathbf{y}  &\text{v.s.}&&
 {H}_1&: \mathbf{P}^{\mathbf{X}_{\eta_1}}\mathbf{y} \neq \mathbf{P}^{\mathbf{X}_{\eta_2}}\mathbf{y}
\end{aligned}
$$
...
$$
\begin{aligned}
 {H}_0&: \mathbf{P}^{\mathbf{X}_{\mu}}\mathbf{y} = \mathbf{P}^{\mathbf{X}_{\mu,\alpha}}\mathbf{y}  &\text{v.s.}&&
 {H}_1&: \mathbf{P}^{\mathbf{X}_{\mu}}\mathbf{y} \neq \mathbf{P}^{\mathbf{X}_{\mu,\alpha}}\mathbf{y}
\end{aligned}
$$
i.e. $ {H}_0$: "factor 1 does not bring any information compared to intercept alone".
...
$$
\begin{aligned}
 {H}_0&: \mathbf{P}^{\mathbf{X}_{\mu,\alpha}}\mathbf{y} = \mathbf{P}^{\mathbf{X}_{\mu,\alpha, \beta}}\mathbf{y}  &\text{v.s.}&&
 {H}_1&: \mathbf{P}^{\mathbf{X}_{\mu,\alpha}}\mathbf{y} \neq \mathbf{P}^{\mathbf{X}_{\mu,\alpha,\beta}}\mathbf{y}
\end{aligned}
$$
i.e. $ {H}_0$: "factor 2 does not bring any information compared to factor 1".
...
$$
\begin{aligned}
 {H}_0&: \mathbf{P}^{\mathbf{X}_{\mu,\alpha,\beta}}\mathbf{y} = \mathbf{P}^{\mathbf{X}_{\mu,\alpha, \beta,\gamma}}\mathbf{y}  &\text{v.s.}&&
 {H}_1&: \mathbf{P}^{\mathbf{X}_{\mu,\alpha,\gamma}}\mathbf{y} \neq \mathbf{P}^{\mathbf{X}_{\mu,\alpha,\beta,\gamma}}\mathbf{y}
\end{aligned}
$$
i.e. $ {H}_0$: "Interactions do not bring any information compared to factors 1 and 2".
...
* $\epsilon_{i, k} \sim  {N}(0, \sigma^2)$ iid noise.
...
* $\mu$ the intercept
* $A_k \sim  {N}(0, s_A^2)$ iid effect of group $k$ on the intercept
* $G_k \sim  {N}(0, s_G^2)$ iid effect of group $k$ on the slope
* $\epsilon_{ik} \sim  {N}(0, \sigma^2)$ iid noise.
...
\quad \epsilon_{i,k} \sim  {N}(0, \sigma^2) ~ iid
$$

$$
 {H}_0: \alpha_1 = \cdots = \alpha_K = 0
$$

**Mixed Model**
$$
y_{i,k} = \mu + A_{k}  + \epsilon_{ik}
\quad A_k \sim  {N}(0, s_A^2) ~ iid
\quad \epsilon_{i,k} \sim  {N}(0, \sigma^2) ~ iid
$$
...
\qquad
\begin{aligned}
\mathbf{U} &\sim  {N}(\mathbf{0}, s_A^2\mathbf{I}_{K}) \\
\boldsymbol{\epsilon} &\sim  {N}(\mathbf{0}, \sigma^2\mathbf{I}_{nK})
\end{aligned}
$$
...
**Mixed Model**
$$
\mathbf{y} \sim  {N}(\mathbf{X}\boldsymbol{\beta}, \mathbf{\Sigma})
$$
...
\qquad
\begin{aligned}
\mathbf{U}_1 &\sim  {N}(\mathbf{0}, s_A^2\mathbf{I}_{K}) \\
\mathbf{U}_2 &\sim  {N}(\mathbf{0}, s_G^2\mathbf{I}_{K}) \\
\boldsymbol{\epsilon} &\sim  {N}(\mathbf{0}, \sigma^2\mathbf{I}_{nK})
\end{aligned}
$$
...
\sim
 {N}(\mathbf{0}, \sigma^2 \mathbf{P}^{\mathbf{X}^\bot})
$$
