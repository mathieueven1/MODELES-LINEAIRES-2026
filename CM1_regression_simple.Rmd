...
-   Mathieu Even (chercheur, Inria)
    -   [mathieu.even\@inria](mailto:mathieu.even@inria){.email}
    -   Statistiques, Inférence causale, ML
-   Amélie Vernay
    -   [amelie.vernay\@umontpellier.fr](mailto:amelie.vernay@umontpellier.fr){.email}
...
-   Voulez vous les slides et quel moyen préférentiel pour les partager?
...
## The Gaussian Setting (teaser for future lessons)

$$y_i = \beta_0 + \beta_1 x_i + \epsilon_i, \quad \forall 1 \leq i \leq n$$

-   $x_i$ multivariate Gaussian
-   $\epsilon_i$ standard univariate and centered Gaussian
...
aka finding the best possible line
...
$(\hat{\beta}_0, \hat{\beta}_1) $ minimizes the loss:
...
-   Question: How can we minimize such a function in general?
-   There is however a closed form solution here: how to compute it?
...
With $$
s_{\mathbf{x}}^2 = \frac{1}{n}\sum_{i = 1}^n (x_i - \bar{x})^2
\qquad
s_{\mathbf{y},\mathbf{x}}^2 = \frac{1}{n}\sum_{i = 1}^n (x_i - \bar{x})(y_i - \bar{y})
...
&= \frac{1}{n s_{\mathbf{x}}^2}\sum_{i = 1}^n (x_i - \bar{x})(\beta_0 + \beta_1x_i + \epsilon_i - [\beta_0 + \beta_1\bar{x}+\overline{\epsilon}])\\
...
>     $$ with $\mathbb{E}[\epsilon_{n+1}] = 0$, $\mathbb{Var}[\epsilon_{n+1}] = \sigma^2$ and $\mathbb{Cov}[\epsilon_{n+1}; \epsilon_i] = 0$.
...
The prediction error $\hat{\epsilon}_{n+1} = y_{n+1} - \hat{y}_{n+1}$ is such that: $$
\begin{aligned}
